...
- `AGENTS.md`, `PROJECT_STATE.md`, `.agents/skills`, `.agents/rules`, `SKILL.md`, `main.py`, `diagram.json`, `pyproject.toml`, `uv.lock`, `.python-version` y `.venv` se conservan como excepciones porque Antigravity, Wokwi/MicroPython o `uv` exigen esos nombres técnicos para el control, descubrimiento de Skills o gestión del entorno.
...
- Se debe usar la Skill `cerrar-modulo-entrevoces` después de completar y verificar un módulo.
