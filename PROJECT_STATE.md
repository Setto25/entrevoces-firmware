...
- `documentacion/GUIA_SESIONES_ANTIGRAVITY.md`: protocolo para iniciar, verificar y cerrar sesiones de Antigravity.
...
- Se instaló MicroPython genérico en la placa XIAO ESP32-S3 Sense y se validó exitosamente el Hito 2 parcial (H2.0, H2.0a, H2.2): conexión REPL, detección de PSRAM, y conexión Wi-Fi.
- Se intentó capturar audio PDM mediante `machine.I2S` estándar (H2.4a), confirmando que inicializa pero produce ruido estático al carecer de decodificación PDM-a-PCM en el firmware base. Esto detona el paso a H2.4b (módulo nativo / custom firmware).
...
Continuar H2 (Hardware por piezas):

1. Planificar y ejecutar H2.4b: el código C del módulo PDM nativo ya está escrito, falta compilar el firmware MicroPython (usando Docker o GitHub Actions) y flashearlo a la placa.
2. Capturar exitosamente una muestra de voz inteligible (completar H2.4).
3. Conectar y validar el pulsador físico con antirrebote (H2.3).
4. Probar la salida I2S TX y conectar el PCM5102A (H2.5).

En paralelo con H2, se puede preparar el backend para persistir las sesiones.
